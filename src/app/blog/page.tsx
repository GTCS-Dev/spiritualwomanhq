  ...
  return (
    <div className="min-h-screen bg-[#001946] text-white antialiased selection:bg-[#980140]/40 selection:text-white relative">
      ...
      <main className="w-full overflow-x-hidden relative z-10">
        ...
        {/* ── HERO POST ── */}
        {heroPost ? (
            <article className="group mx-auto mt-10 grid w-full max-w-6xl overflow-hidden rounded-3xl border border-white/[0.08] bg-[#001233]/80 backdrop-blur-sm shadow-[0_20px_40px_-28px_rgba(0,0,0,0.4)] transition-all duration-500 hover:shadow-[0_28px_60px_-24px_rgba(152,1,64,0.3)] lg:grid-cols-[1.1fr_0.9fr]">
            <div className="relative overflow-hidden">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#001233]/60 via-transparent to-transparent lg:hidden" />
            </div>
              <div className="flex flex-col justify-between gap-6 p-8 sm:p-12">
                <div>
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-[#980140]/15 px-3.5 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-[#E19508]">
                    <span className="h-1.5 w-1.5 rounded-full bg-[#E19508]" />
                    {categoryLabels[heroPost.category]}
                  </span>
                  <h2 className="mt-4 text-3xl font-semibold leading-tight text-white sm:text-4xl">{heroPost.title}</h2>
                  <p className="mt-4 text-sm leading-7 text-white/70 sm:text-[0.95rem] sm:leading-8 line-clamp-3">{heroPost.excerpt}</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Link
                    ...
                    className="inline-flex rounded-full bg-gradient-to-r from-[#980140] to-[#A2014A] px-6 py-3 text-sm font-bold text-white transition-all duration-300 hover:shadow-[0_6px_20px_-8px_rgba(152,1,64,0.5)] hover:scale-105"
                  >
                    Read Article
                  </Link>
                </div>
        ...
        <section className="mx-auto mt-10 grid w-full max-w-6xl gap-6 px-4 sm:px-6 md:grid-cols-2 lg:grid-cols-3 lg:px-8">
          {gridPosts.map((post) => (
            <article key={post.id} className="group overflow-hidden rounded-2xl border border-white/[0.08] bg-[#001233]/80 backdrop-blur-sm shadow-[0_12px_30px_-20px_rgba(0,0,0,0.3)] transition-all duration-400 hover:-translate-y-1.5 hover:border-[#E19508]/30 hover:shadow-[0_20px_50px_-20px_rgba(152,1,64,0.2)]">
              <div className="relative overflow-hidden">
              ...
              <div className="px-6 py-6">
                ...
                <p className="mt-3 text-sm leading-7 text-white/60 line-clamp-2">{post.excerpt}</p>
                <Link href={`/blog/${post.slug}`} className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#E19508] transition-all duration-300 hover:gap-3 group/link">
